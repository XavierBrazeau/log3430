 ...
 ***********************************************************************************/
	...
	void testVConstrV() {
		...
		//Test reverse
		assertEquals(digraph.reverse().V(), vertex);
		assertEquals(digraph.V(), vertex); //Pour s'assurer que notre variable vertex n'a pas �t� modifi�
	}
	...
	void testVConstrCopy() {
		...
		String expectedPrefix = String.valueOf(vertex);
		assertTrue(digraph.toString().startsWith(expectedPrefix));
		assertEquals(digraph.V(), vertex);	// V�rifie que notre graphe n'a pas chang�.
 ...
 ***********************************************************************************/
	...
	void testAdjConstrVAddEdgeReverse() {
		...
		//le code suivant est r�utilis� pour s'assurer que la fonction reverse n'a pas
		//alter� l'objet
		for(int i = 0; i < vertex; i++) {
			if(i == vStart) {
				Iterator<Integer> it = digraph.adj(i).iterator();
		...
		//Test outDegree
		for(int i = 0 ; i < vertex; i++) {
			assertEquals(digraph.outdegree(i), out(digraph.adj(i)));
		}
 ...
 ***********************************************************************************/
	...
	void testIndegreeConstrVAddEdgeReverse() {
		// Test Constructor V & Getter indegree
		Digraph digraph = new Digraph(vertex);
		for(int i = 0; i < vertex; i++)
	...
	void testIndegreeConstrInAddEdgeReverse() {
		...
		// Test Constructor In & Getter indegree
		Digraph digraph = new Digraph(in);
		for(int i = 0; i < vertex; i++)
			assertEquals(digraph.indegree(i), 0);
